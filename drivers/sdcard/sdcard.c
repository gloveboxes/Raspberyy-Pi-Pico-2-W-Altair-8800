...
#ifdef SDCARD_PIO
pio_spi_inst_t pio_spi = {.pio = SDCARD_PIO, .sm = SDCARD_PIO_SM};
#endif
...
{
    // Debug: Print actual pin values being used
    printf("[SD] SPI init: SCK=%d, MISO=%d, MOSI=%d, CS=%d\n", SDCARD_PIN_SPI0_SCK, SDCARD_PIN_SPI0_MISO,
           SDCARD_PIN_SPI0_MOSI, SDCARD_PIN_SPI0_CS);
    printf("[SD] SPI bus: %s\n", (SDCARD_SPI_BUS == spi1) ? "spi1" : "spi0");
    ...
    spi_set_format(SDCARD_SPI_BUS, 8, /* data_bits */
                   SPI_CPOL_0,        /* cpol */
                   SPI_CPHA_0,        /* cpha */
                   SPI_MSB_FIRST      /* order */
    );
    ...
    pio_spi_init(pio_spi.pio, pio_spi.sm, cpha0_prog_offs,
                 8, // 8 bits per SPI frame
                 clkdiv, cpha, cpol, SDCARD_PIN_SPI0_SCK, SDCARD_PIN_SPI0_MOSI, SDCARD_PIN_SPI0_MISO);
#endif
}
...
static void xmit_spi_multi(const BYTE* buff, /* Pointer to data buffer */
